/**
 * Static PNG bake for TOC bookmark: deckled rim frame only (transparent interior).
 *
 * The row container supplies the highlight surface (incl. hover), and the rim is baked in the same
 * color so the frame reads as part of the container instead of an independently filled chip.
 */
import * as THREE from 'three'

export type TocBookmarkRimBakeOptions = {
  widthCss: number
  heightCss: number
  colorHex: string
  bleedPx?: number
...
/** On 8px grid (2 × 8px) — generous for angled bookmark notch in atlas */
export const TOC_BOOKMARK_BLEED_PX = 16

const MAX_BAKE_EDGE_PX = 1024

const BOOKMARK_CHISEL_BAKE_FRAGMENT = /* glsl */ `
  ...
  /** Rim band around bookmark silhouette (same idea as rectangular abs(sdBox) minus border on cards). */
  float getBookmarkRimDistance(vec2 p) {
    vec2 q = p - u_pCardCenter;
    float aspectPin = u_resolution.x / max(u_resolution.y, 1.0);
    vec2 bookmarkHalf = vec2(aspectPin * 1.0, 1.0);
    float cutDepth = 0.15 * (bookmarkHalf.x / 0.7);

    float dBm = sdBookmarkHorizontal(q, bookmarkHalf, cutDepth);
    float rim = abs(dBm) - u_borderWidth;
    float dOut = max(0.0, dBm);
  ...
  void main() {
    ...
    float dFinal = getBookmarkRimDistance(p);
    ...
    vec3 rimCol = u_color * alphaMult;

    float rimAlpha = u_flatRim > 0.5
      ? 1.0 - smoothstep(-0.004, 0.016, dFinal)
      : 1.0 - smoothstep(0.0, 0.01, dFinal);

    if (u_depthEffect > 0.0) {
      vec2 bh = vec2(aspectPin * 1.0, 1.0);
      float edgeScale = 0.35 / max(bh.x, bh.y);
      float dLight = getBookmarkRimDistance(p + vec2(0.005, -0.005) * edgeScale);
      ...
      rimCol += vec3(0.6) * highlight * u_depthEffect * rimAlpha;
      rimCol -= vec3(0.6) * selfShadow * u_depthEffect * rimAlpha;
    }

    gl_FragColor = vec4(rimCol, rimAlpha);
  }
`
...
function ensureBookmarkBakeGl() {
  ...
  bakeMaterial = new THREE.ShaderMaterial({
    uniforms,
    vertexShader,
    fragmentShader: BOOKMARK_CHISEL_BAKE_FRAGMENT,
    transparent: true,
    depthWrite: false,
    toneMapped: false,
  })
...
/** Transparent PNG: deckled rim frame (+ bleed outside silhouette). */
export function bakeTocBookmarkRimImage(opts: TocBookmarkRimBakeOptions): string | null {
  const widthCss = opts.widthCss
  const heightCss = opts.heightCss
  if (widthCss < 2 || heightCss < 2)
    return null

  ensureBookmarkBakeGl()
  if (!bakeRenderer || !bakeMaterial || !bakeScene || !bakeCamera)
    return null

  const bleed = opts.bleedPx ?? TOC_BOOKMARK_BLEED_PX
