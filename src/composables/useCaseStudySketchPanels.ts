...
/** Stable seeds per element kind — consistent stroke, cache-friendly. */
const PENCIL_SEED: Record<BoundKind, number> = {
  panel: 11,
  'text-divider-h': 23,
  'divider-v': 37,
  chip: 53,
...
function shouldBindPanel(el: HTMLElement): boolean {
  if (!el.classList.contains('panel-recessed'))
    return false
  if (el.classList.contains('panel-recessed--no-pencil-frame'))
    return false
  if (el.tagName === 'IMG')
    return false
...
export function useCaseStudySketchPanels(rootRef: Ref<HTMLElement | null>) {
  ...
  function rebake(el: HTMLElement, attempt = 0) {
    ...
    if (entry.bakeKey === bakeKey && el.classList.contains(PENCIL_BAKED_CLASS))
      return
  ...
  function bind(el: HTMLElement, kind: BoundKind, observeEl?: HTMLElement) {
    ...
    bound.set(el, { kind, ro: null, timer: 0, blobUrl: null, bakeKey: null })
    rebake(el)
